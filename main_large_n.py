...
for n_nodes in n_nodes_array:
    ...
    for _ in tqdm(np.arange(4)):
        ...
        Selector_object = SubgraphSelector_Offline(ID=ID, SBMs=SBMs, n_subgraphs=n_subgraphs,
                                                   ...
                                                   n_clusters=n_clusters, parent_alg='HC')
    ...
    try:
        results_df = pd.concat([results_df, PACE_results], ignore_index=True)
    except NameError:
        results_df = pd.concat([PACE_results], ignore_index=True)
